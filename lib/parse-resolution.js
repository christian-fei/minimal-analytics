...
module.exports = function parseResolution (url = '') {
  ...
  if (['minutes', 'monthly'].includes(resolution) && /(past-week)/.test(url)) return 'daily'
  if (['minutes', 'monthly'].includes(resolution) && /(past-month)/.test(url)) return 'daily'
  if (['minutes', 'hourly', 'monthly'].includes(resolution) && /(past-6-months)/.test(url)) return 'daily'
