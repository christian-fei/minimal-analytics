...
test('calculate pages breakdown', t => {
...
test('calculate referrers breakdown', t => {
...
test('handles empty chart data', t => {
  t.deepEqual(analytics.calculateChartData([], 'minutes'), [])
  t.deepEqual(analytics.calculateChartData([], 'hourly'), [])
})

test('calculate chart data (minutes)', t => {
  t.snapshot(analytics.calculateChartData([
    { r: 'https://www.google.com/', p: '/posts/2020-03-07-Twitter-OAuth-Login-with-fastify-and-Node.js/', w: 1920, type: 'pageview', v: '3128e344d7', u: 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:87.0) Gecko/20100101 Firefox/87.0', d: new Date().toISOString() }
  ], 'minutes'))
})
test('calculate chart data (hourly)', t => {
  t.snapshot(analytics.calculateChartData([
    { r: 'https://www.google.com/', p: '/posts/2020-03-07-Twitter-OAuth-Login-with-fastify-and-Node.js/', w: 1920, type: 'pageview', v: '3128e344d7', u: 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:87.0) Gecko/20100101 Firefox/87.0', d: new Date().toISOString() }
  ], 'hourly'))
})
