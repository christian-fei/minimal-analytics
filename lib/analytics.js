...
function calculateChartData (data, resolution) {
  const mapped = data.reduce((acc, curr) => {
    ...
    const existing = acc.find(a => a[0] === normalizedDate)
    ...
    return acc.concat([[normalizedDate, 1]])
  }, [])
  ...
  for (let s = normalizeDate(Date.now(), resolution); new Date(s) > new Date(mapped[mapped.length - 1][0]); s = normalizeDate(+new Date(s) - step, resolution)) {
    const existing = mapped.find(a => a[0] === s)
    if (existing) {
