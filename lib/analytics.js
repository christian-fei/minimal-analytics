...
function getAll (url, memory, live) {
  ...
  const data = memory.filter(m =>
    ...
    (!!filters.s ? ((m.p || '').toLowerCase().includes(filters.s.toLowerCase()) || (m.v || '').toLowerCase().includes(filters.s.toLowerCase()) || (m.r || '').toLowerCase().includes(filters.s.toLowerCase())) : (keys.length === 0 || keys.every(k => m[k] === filters[k])))
  ).reverse()

  const pages = calculatePagesBreakdown(data, 20)
  const events = calculateEventsBreakdown(data, 20)
...
function calculateEventsBreakdown (data, max) {
  return data.reduce((acc, curr) => {
    if (curr.t !== 'event') return acc
    if (acc.find(a => a.e === curr.e)) {
      return acc.map((a) => {
        if (a.e === curr.e) {
          a.views++
          return a
    ...
    return acc.concat([{ e: curr.e, views: 1 }])
  }, [])
    .sort((a, b) => b.views - a.views)
    .filter((d, i) => !max || i < max)
}
