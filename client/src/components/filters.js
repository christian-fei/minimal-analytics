export default function ({ updateTimeframe, updateResolution, filters }) {
  return (
    <div class='grid-lg contain'>
      ...
      <div class='w-50-lg'>
        <h4>Resolution</h4>
        <span onClick={() => updateResolution('hourly')} class={`select-resolution filterable ${filters.resolution === 'hourly' && 'active'}`} name='hourly' id='hourly'>Hourly</span>
        {['past-week', 'past-month'].includes(filters.timeframe) && <span onClick={() => updateResolution('daily')} class={`select-resolution filterable ${filters.resolution === 'daily' && 'active'}`} name='daily' id='daily'>Daily</span>}
